...
public class DatabaseOperations extends AsyncTask<String,Void,String> {
    ...
    protected String doInBackground(String... params) {
        ...
        if(type.equals("loginPatient")) {
            try {
                String cpr = params[1];
                String password = params[2];
                URL url = new URL("http://7da6074e.ngrok.io/loginPatient.php");
                HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
                httpURLConnection.setRequestMethod("POST");
                httpURLConnection.setDoOutput(true);
                httpURLConnection.setDoInput(true);
        ...
        if(type.equals("loginPractitioner")) {
            try {
                String clinicID = params[1];
                String employmentID = params[2];
                String password = params[3];
                URL url = new URL("http://7da6074e.ngrok.io/loginPractitioner.php");
                HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
                httpURLConnection.setRequestMethod("POST");
                httpURLConnection.setDoOutput(true);
                httpURLConnection.setDoInput(true);
                OutputStream outputStream = httpURLConnection.getOutputStream();
                BufferedWriter bufferedWriter = new BufferedWriter(new OutputStreamWriter(outputStream, "UTF-8"));
                String post_data = URLEncoder.encode("clinicID", "UTF-8") + "=" + URLEncoder.encode(clinicID, "UTF-8") + "&"
                        + URLEncoder.encode("employmentID", "UTF-8") + "=" + URLEncoder.encode(employmentID, "UTF-8") + "&"
                        + URLEncoder.encode("password", "UTF-8") + "=" + URLEncoder.encode(password, "UTF-8");
                bufferedWriter.write(post_data);
                bufferedWriter.flush();
                bufferedWriter.close();
